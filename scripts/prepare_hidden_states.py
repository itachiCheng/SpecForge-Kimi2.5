...
def parse_args():
    ...
    inference_group.add_argument(
        "--debug-logits",
        action="store_true",
        help="Print a small logits-to-token sanity check for a few batches.",
    )
    inference_group.add_argument(
        "--debug-logits-batches",
        type=int,
        default=2,
        help="How many batches to print when --debug-logits is enabled.",
    )
...
def build_target_model(
    ...
    runtime_device_type = runtime_device.type
...
class HiddenStatesGenerator:
    ...
    def __init__(
        ...
        self.is_vlm = is_vlm
        self.device = _get_runtime_device()
        ...
        self.debug_logits_batches = debug_logits_batches
    ...
    def _debug_print_logits(
        ...
        try:
            logits_tensor = _first_tensor(logits_list)
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            if is_tp_rank_0():
                ...
                exists_tensor = torch.tensor(
                    exists_list, dtype=torch.bool, device=self.device
                )
            else:
                exists_tensor = torch.tensor(
            ...
            filtered_batch_gpu = {
                k: v.to(self.device, non_blocking=True) for k, v in filtered_batch.items()
            }
            if self.is_vlm and (
                "pixel_values" in filtered_batch_gpu
                and ("image_grid_thw" in filtered_batch_gpu or "grid_thws" in filtered_batch_gpu)
            ):
                _, logits_list, aux_hidden_states_list, last_hidden_states_list = (
                ...
                _, logits_list, aux_hidden_states_list, last_hidden_states_list = self.model.extend(
                    input_ids=filtered_batch_gpu["input_ids"],
                    attention_mask=filtered_batch_gpu["attention_mask"],
                    loss_mask=filtered_batch_gpu["loss_mask"],
                    return_last_hidden_states=True,
            ...
            self._debug_print_logits(batch_idx, filtered_batch, logits_list)
...
def main():
    ...
    cache_params_string = f"{args.data_path}-{args.max_length}-{args.chat_template}-{args.target_model_path}-{args.num_samples}-{args.is_preformatted}-vlm_media_pad_expand_v4_grid_aligned_multimodal_content_collapse_empty_think"
    ...
    try:
        ...
        with HiddenStatesGenerator(
            ...
            debug_logits_batches=args.debug_logits_batches,
