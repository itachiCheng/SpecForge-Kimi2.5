...
def _get_current_accelerator_index() -> int:
...
class SGLangEagle3TargetModel(Eagle3TargetModel):
    ...
    def _init_vlm_attributes(self):
        ...
        if isinstance(merge_kernel_size, (list, tuple)) and len(merge_kernel_size) == 2:
            self.mm_merge_area = int(merge_kernel_size[0]) * int(merge_kernel_size[1])
        else:
            self.mm_merge_area = int(self.spatial_merge_size) * int(self.spatial_merge_size)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model_runner = SGLangRunner(
            ...
            gpu_id=_get_current_accelerator_index(),
            tp_rank=dist.get_rank(get_tp_group()),
    ...
    def _compute_kimi_placeholder_tokens(self, image_grid_thw: torch.Tensor) -> int:
        """
        Kimi-K2.5 vision tower uses spatial merge + temporal pooling.
        Placeholder token count should match post-merge vision tokens.
        """
        ...
        merge_kernel_size = getattr(self, "merge_kernel_size", (2, 2))
        if not (
            isinstance(merge_kernel_size, (list, tuple)) and len(merge_kernel_size) == 2
        ):
            merge_kernel_size = (2, 2)
        kh = max(1, int(merge_kernel_size[0]))
        kw = max(1, int(merge_kernel_size[1]))

        merged_token_count = 0
        for row in image_grid_thw.tolist():
            # row = [t, h, w]; Kimi tpool patch merger collapses temporal dim.
            _t, h, w = int(row[0]), int(row[1]), int(row[2])
            merged_h = max(1, h // kh)
            merged_w = max(1, w // kw)
            merged_token_count += merged_h * merged_w
        return max(1, int(merged_token_count))
    ...
    def extend_vlm(
        ...
        for idx, (input_id_, attention_mask_, loss_mask_, image_grid_thw_) in enumerate(
            ...
            if is_kimi_k25 and num_patches > 0 and len(offset) == 1:
                merged_token_count = self._compute_kimi_placeholder_tokens(image_grid_thw_)
                start_idx, end_idx = offset[0]
                current_token_count = end_idx - start_idx + 1
                if current_token_count != merged_token_count:
                    offset = [(start_idx, start_idx + merged_token_count - 1)]
                    logger.info(
                        "[KimiVLM] sample=%s grid=%s num_patches=%s merge_kernel=%s placeholder_tokens=%s",
                        idx,
                        image_grid_thw_.tolist(),
                        num_patches,
                        getattr(self, "merge_kernel_size", None),
                        merged_token_count,
                    )
