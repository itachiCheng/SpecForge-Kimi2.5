...
def _get_offsets_for_loss_mask(processor, encoding, decoded_conversation: str):
    if "offset_mapping" in encoding:
        return encoding.offset_mapping[0]
    try:
        ...
        return torch.tensor(tokenized["offset_mapping"], dtype=torch.long)
...
def _apply_loss_mask_from_token_ids(
    ...
    assistant_sep = (
        chat_template.end_of_turn_token + chat_template.assistant_header
    )
    user_sep = chat_template.end_of_turn_token + chat_template.user_header
    assistant_sep_ids = tokenizer.encode(assistant_sep, add_special_tokens=False)
    user_sep_ids = tokenizer.encode(user_sep, add_special_tokens=False)

    if not assistant_sep_ids:
        return loss_mask

    assistant_starts = _find_subsequence_positions(seq, assistant_sep_ids)
    user_starts = _find_subsequence_positions(seq, user_sep_ids) if user_sep_ids else []

    for start in assistant_starts:
        content_start = start + len(assistant_sep_ids)
        content_end = len(seq)
...
def preprocess_vlm_conversations(
    ...
    if is_preformatted:
        ...
        for text, image in zip(examples["text"], images):
            ...
            offsets = _get_offsets_for_loss_mask(
                processor, encoding, decoded_conversation
            )
            if offsets is not None:
                ...
                loss_mask = _apply_loss_mask_from_token_ids(
                    input_ids, processor.tokenizer, chat_template
                )
